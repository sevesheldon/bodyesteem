...
class Admin_Powerkit_Manager {
	...
	public function settings_page() {
		...
		// Filter modules.
		if ( isset( $_GET['filter'] ) ) { // Input var ok.
			$filter = sanitize_key( $_GET['filter'] ); // Input var ok.
		}
		...
		<div class="wrap powerkit-manager">
			<h2><?php esc_html_e( 'Powerkit Modules', 'powerkit' ); ?></h2>
	...
	public function handler_actions() {
		...
		if ( isset( $_REQUEST['checked'] ) && is_array( $_REQUEST['checked'] ) ) { // Input var ok; sanitization ok.
			$checked = $_REQUEST['checked']; // Input var ok; sanitization ok.
