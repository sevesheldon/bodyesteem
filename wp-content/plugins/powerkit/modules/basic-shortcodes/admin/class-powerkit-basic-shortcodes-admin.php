 ...
 * The admin-specific functionality of the module.
 */
class Powerkit_Basic_Shortcodes_Admin extends Powerkit_Module_Admin {
	...
	public function powerkit_basic_shortcodes_register_plugin( $plugins ) {
		if ( current_user_can( 'edit_posts' ) ) {
			$plugins['powerkit_basic_shortcodes'] = trailingslashit( plugin_dir_url( __FILE__ ) ) . 'js/admin-powerkit-basic-shortcodes.js';
		}
						...
						(function($) {
							...
							$('.tabs-<?php echo esc_attr( $section['name'] ); ?> form').submit( function( e ) {
								...
								<?php foreach ( (array) $section['fields'] as $field ) : ?>
									...
									var fieldName = '<?php echo esc_attr( $field['name'] ); ?>';
									<?php
									switch ( $field['type'] ) {
										case 'section':
											break;
	...
	public function admin_enqueue_scripts( $page ) {
		...
		// Styles.
		wp_enqueue_style( 'powerkit-basic-shortcodes', plugin_dir_url( __FILE__ ) . 'css/admin-powerkit-basic-shortcodes.css', array(), powerkit_get_setting( 'version' ), 'all' );
