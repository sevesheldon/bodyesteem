...
class Powerkit_Custom_Fonts extends Powerkit_Module {
	...
	public function frontend_enqueue() {
		...
		if ( is_array( $custom_fonts ) && $custom_fonts ) {
			...
			foreach ( $custom_fonts as $key => $item ) {
				...
				if ( $id ) {
					...
					if ( $src ) {
						...
						$font_face .= sprintf( '@font-face { font-family: "%1$s"; src: %2$s; font-weight: %3$s; font-style: %4$s;}', $id, $src_line, $item['weight'], $item['style'] );
					}
				}
			}
			...
			if ( $font_face ) {
				?>
					<style><?php echo print( $font_face ); // XSS. ?></style>
				<?php
			}
		}
	}
