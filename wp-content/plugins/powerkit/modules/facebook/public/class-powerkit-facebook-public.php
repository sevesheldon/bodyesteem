...
class Powerkit_Facebook_Public extends Powerkit_Module_Public {
	...
	public function register_locations() {
		...
		if ( $powerkit_facebook_enable_comments && $location ) {
			$priority = (int) isset( $location['priority'] ) ? $location['priority'] : 10;
			...
			if ( 'comments_template' === $location['action'] ) {
				...
				function powerkit_filter_comments_template() {
					remove_filter( 'comments_template', 'powerkit_filter_comments_template' );

					comments_template();

					return plugin_dir_path( dirname( __FILE__ ) ) . 'public/facebook-comments-template.php';
				}
				add_filter( 'comments_template', 'powerkit_filter_comments_template' );
	...
	public function wp_enqueue_scripts() {
		wp_enqueue_style( 'powerkit-facebook', plugin_dir_url( __FILE__ ) . 'css/public-powerkit-facebook.css', array(), powerkit_get_setting( 'version' ), 'all' );
