...
class Powerkit_Lazyload_Public extends Powerkit_Module_Public {
	...
	public function initialize() {
		...
		$placeholder_image = powerkit_get_image_placeholder( 1, 1, true );
	...
	public function generate_attachment_placeholder( $metadata ) {
		...
		if ( $placeholder_unique ) {
			...
			if ( isset( $metadata['width'] ) && isset( $metadata['height'] ) ) {
				$metadata['placeholder'] = powerkit_get_image_placeholder( $metadata['width'], $metadata['height'] );
			}
			...
			if ( isset( $metadata['sizes'] ) ) {
				foreach ( $metadata['sizes'] as $slug => & $size ) {
					...
					if ( isset( $size['width'] ) && isset( $size['height'] ) ) {
						$size['placeholder'] = powerkit_get_image_placeholder( $size['width'], $size['height'] );
					}
				}
			}
		}
	...
	public function content_process_images( $content ) {
		...
		foreach ( $images as $image ) {
			// Replace single paws.
			$kses_image = str_replace( "'", '"', $image );

			// Ignore init lazyload.
			if ( preg_match( '/pk-lazyload/', $kses_image ) ) {
				continue;
			}

			// Get Attributes for the image markup.
			if ( preg_match_all( '/\s(.*?)="(.*?)"/', $kses_image, $matches ) ) {
				 ...
				 * --------------------------------
				 */
				$attachment = powerkit_attachment_attr_to_object( $attr );
				$size       = powerkit_attachment_attr_to_size( $attr );
	...
	public function add_image_placeholders( $attr, $attachment, $size ) {
		...
		if ( is_string( $size ) ) {
			...
			if ( $this->is_lqip_enabled( $attr ) ) {
				...
				if ( powerkit_get_image_size( $lqip ) ) {
	...
	public function wp_enqueue_scripts() {
		if ( $this->is_enabled() ) {
			...
			wp_enqueue_style( 'powerkit-lazyload', plugin_dir_url( __FILE__ ) . 'css/public-powerkit-lazyload.css', array(), powerkit_get_setting( 'version' ), 'all' );
		}
	}
}
