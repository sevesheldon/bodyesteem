...
class Powerkit_Opt_In_Forms_Public extends Powerkit_Module_Public {
	...
	public function register_subscription_form( $params = array() ) {
		...
		if ( $token ) {
			...
			if ( $params['list_id'] ) {
					...
					<div class="pk-subscribe-container <?php echo esc_attr( $params['bg_image_id'] ? ' pk-bg-overlay' : '' ); ?>">
						<div class="pk-subscribe-data">
							...
							<form method="post" class="subscription">
								...
								<input type="hidden" name="_wpnonce" value="<?php echo esc_attr( wp_create_nonce( -1 ) ); ?>" />
				...
				powerkit_alert_warning( sprintf( __( 'Please select the "List" for your subscription form in <a href="%s" target="_blank">Opt-In Forms Settings</a>.', 'powerkit' ), esc_url( powerkit_get_page_url( 'opt_in_forms' ) ) ) );
			}
		} else {
			/* translators: MailChimp Settings. */
			powerkit_alert_warning( sprintf( __( 'Please add your MailChimp Token in <a href="%s" target="_blank">Opt-In Forms Settings</a>.', 'powerkit' ), esc_url( powerkit_get_page_url( 'opt_in_forms' ) ) ) );
		}
	}
	...
	public function subscription() {
		...
		if ( isset( $_POST['USER'] ) ) { // Input var ok.
			$user = sanitize_text_field( wp_unslash( $_POST['USER'] ) ); // Input var ok.
		}
	...
	public function wp_enqueue_scripts() {
		// Styles.
		wp_enqueue_style( 'powerkit-opt-in-forms', plugin_dir_url( __FILE__ ) . 'css/public-powerkit-opt-in-forms.css', array(), powerkit_get_setting( 'version' ), 'all' );
