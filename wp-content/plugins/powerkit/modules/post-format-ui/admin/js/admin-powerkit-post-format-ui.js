...
( function( $ ) {
	...
	*/
	$( function() {
		...
		*/
		$( document ).on( 'click', '.post-format', function( event ) {
		...
		*/
		function powerkitUIGalleryIds( action, id ) {
			let obj = $( '.pk-post-format-gallery-settings' );
			...
			*/
			...
			galleryModal.on( 'select', function() {
				...
				for ( var prop in attachments ) {
					...
					let attachment_id  = attachments[prop].id;
					let attachment_src = attachments[prop].url;
					...
					// Add attachment.
					let attachment = $( '<div class="pk-post-format-gallery-attachment"></div>' ).appendTo( '.pk-post-format-gallery-attachments' );
