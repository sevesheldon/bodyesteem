...
class Powerkit_Share_Buttons_Admin extends Powerkit_Module_Admin {
	...
	public function register_options_default() {
		...
		if ( $locations ) {
			foreach ( $locations as $key => $item ) {
				...
				add_filter( "default_option_powerkit_share_buttons_{$location}_display", function ( $default ) use ( $item ) {
					return isset( $item['display'] ) ? $item['display'] : $default;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_multiple_list", function ( $default ) use ( $item ) {
					return isset( $item['shares'] ) ? (array) $item['shares'] : $default;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_order_multiple_list", function ( $default ) use ( $item ) {
					return isset( $item['shares'] ) ? (array) $item['shares'] : $default;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_display_labels", function ( $default ) use ( $item ) {
					return isset( $item['meta']['labels'] ) ? $item['meta']['labels'] : true;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_display_total_share_count", function ( $default ) use ( $item ) {
					return isset( $item['display_total'] ) ? $item['display_total'] : true;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_display_count", function ( $default ) use ( $item ) {
					return isset( $item['display_count'] ) ? $item['display_count'] : true;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_count_location", function ( $default ) use ( $item ) {
					return isset( $item['count_location'] ) ? $item['count_location'] : $default;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_layout", function ( $default ) use ( $item ) {
					return isset( $item['layout'] ) ? $item['layout'] : $default;
				} );
				add_filter( "default_option_powerkit_share_buttons_{$location}_scheme", function ( $default ) use ( $item ) {
	...
	public function build_options_page() {
		...
		?>
			...
			<div class="wrap pk-share-buttons-wrap">
				<h2><?php esc_html_e( 'Share Buttons', 'powerkit' ); ?></h2>
					...
					<?php if ( $locations ) : ?>
						<form class="pk-share-buttons-basic" method="post">
							<div class="pk-share-buttons-tabs">
								...
								$tab = isset( $_GET['tab'] ) ? esc_attr( $_GET['tab'] ) : $locations[0]['location']; // Input var ok; sanitization ok.
								?>
									...
									<div id="tab-<?php echo esc_attr( $location ); ?>" class="tab-wrap">
										<table class="form-table">
											<tbody>
												...
												<!-- Layout -->
												<?php
												$layouts = powerkit_share_buttons_format_layouts( $item );
															...
															<select class="regular-text" name="powerkit_share_buttons_<?php echo esc_attr( $location ); ?>_layout" id="powerkit_share_buttons_<?php echo esc_attr( $location ); ?>_layout">
																...
																if ( $layouts ) {
																	foreach ( $layouts as $key => $item ) {
																		?>
																			<option value="<?php echo esc_attr( $key ); ?>" <?php selected( get_option( "powerkit_share_buttons_{$location}_layout" ), $key ); ?>><?php echo esc_attr( $item['name'] ); ?></option>
																		<?php
																	}
																}
																?>
															...
															<select class="regular-text" name="powerkit_share_buttons_<?php echo esc_attr( $location ); ?>_scheme" id="powerkit_share_buttons_<?php echo esc_attr( $location ); ?>_scheme">
																...
																if ( $schemes ) {
																	foreach ( $schemes as $key => $item ) {
																		?>
																			<option value="<?php echo esc_attr( $key ); ?>" <?php selected( get_option( "powerkit_share_buttons_{$location}_scheme" ), $key ); ?>><?php echo esc_attr( $item['name'] ); ?></option>
																		<?php
																	}
																}
																?>
	...
	protected function save_options_page() {
		...
		if ( isset( $_POST['save_settings'] ) ) { // Input var ok.
			...
			if ( $locations ) {
				foreach ( $locations as $key => $item ) {
					...
					if ( isset( $_POST[ "powerkit_share_buttons_{$location}_multiple_list" ] ) ) { // Input var ok.
						update_option( "powerkit_share_buttons_{$location}_multiple_list", $_POST[ "powerkit_share_buttons_{$location}_multiple_list" ] ); // Input var ok; sanitization ok.
					} else {
					...
					if ( isset( $_POST[ "powerkit_share_buttons_{$location}_order_multiple_list" ] ) ) { // Input var ok.
						update_option( "powerkit_share_buttons_{$location}_order_multiple_list", (array) $_POST[ "powerkit_share_buttons_{$location}_order_multiple_list" ] ); // Input var ok; sanitization ok.
					} else {
			...
			// Reset cache.
			Powerkit_Connect::reset_cache( array(
				'powerkit_share_buttons_count',
				'powerkit_share_buttons_transient',
			) );
	...
	public function admin_enqueue_scripts( $page ) {
		if ( 'settings_page_' . powerkit_get_page_slug( $this->slug ) === $page ) {
			...
			// Styles.
			wp_enqueue_style( 'powerkit-share-buttons', plugin_dir_url( __FILE__ ) . 'css/admin-powerkit-share-buttons.css', array(), powerkit_get_setting( 'version' ), 'all' );
