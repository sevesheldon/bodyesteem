...
function powerkit_share_buttons_make_bitly_url( $api_token, $url, $title = null ) {
	...
	$data = wp_remote_post( 'https://api-ssl.bitly.com/v4/bitlinks', array(
		'method'  => 'POST',
		'headers' => array(
 ...
 */
function powerkit_share_buttons_get_cached_account( $account, $post_id, $url = null, $igonre_time = false, $suffix = false ) {
 ...
 * @param bool       $igonre_time  Ignore Cache Time.
 */
function powerkit_share_buttons_get_cached_count( $account, $post_id, $url = null, $igonre_time = false ) {
	$count = powerkit_share_buttons_get_cached_account( $account, $post_id, $url, $igonre_time );

	// Recover shares.
	if ( get_option( 'powerkit_share_buttons_recover', false ) ) {
		$alt_count = powerkit_share_buttons_get_cached_account( $account, $post_id, $url, $igonre_time, 'recover' );
...
function powerkit_share_buttons_get_count( $account, $post_id = false, $url = null ) {
	...
	if ( isset( $account_data['count_walker'] ) ) {
		...
		if ( function_exists( $func ) ) {
			$count = call_user_func_array( $func, array( $account, $post_id, $url ) );

			// Recover shares.
			if ( get_option( 'powerkit_share_buttons_recover', false ) ) {
				$url = powerkit_share_buttons_get_alt_url( $url );

				$count += call_user_func_array( $func, array( $account, $post_id, $url, 'recover' ) );
			}
		}
	}
 ...
 * @param string $url            Custom URL.
 */
function powerkit_share_buttons( $accounts = array( 'facebook', 'twitter', 'googleplus' ), $total = true, $icons = true, $titles = false, $labels = true, $counts = true, $count_location = 'inside', $mode = 'mixed', $layout, $scheme = 'default', $class = null, $url = null ) {
	...
	?>
		<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" data-post-id="<?php echo esc_attr( $post_id ); ?>" data-share-url="<?php echo esc_url( powerkit_share_buttons_get_url( $post_id, $url ) ); ?>">
				...
				foreach ( $accounts as $account ) {
					...
					?>
						<div class="<?php echo esc_attr( implode( ' ', $item_classes ) ); ?>" data-id="<?php echo esc_attr( $account ); ?>">
							...
							<a href="<?php echo esc_url( $account_data['share_url'], null, '' ); ?>" class="pk-share-buttons-link" target="_blank">
 ...
 */
function powerkit_share_buttons_format_layouts( $location ) {
...
function powerkit_share_buttons_handler_layout( $location_name ) {
	...
	foreach ( $locations as $item ) {
		if ( $location_name === $item['location'] ) {
			...
			$format_layouts = powerkit_share_buttons_format_layouts( $item );
...
function powerkit_share_buttons_location( $location = 'after-content', $url = null ) {
	...
	if ( $display_share ) {
		...
		// Get Shares.
		powerkit_share_buttons( $accounts, $total, $icons, $titles, $labels, $counts, $count_location, $mode, $layout, $scheme, $class, $url );
