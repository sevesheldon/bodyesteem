...
class Powerkit_Slider_Gallery_Public extends Powerkit_Module_Public {
	...
	public function gallery_settings( $settings, $attr ) {
		...
		if ( isset( $attr['type'] ) && 'slider' === $attr['type'] ) {
			...
			$settings['custom_attrs'] .= sprintf( ' data-sg-page-dots="%s" data-sg-page-info="%s" data-sg-nav="%s"', $data['pk-sg-page-dots'], $data['pk-sg-page-info'], $data['pk-sg-nav'] );
		}
	...
	public function wp_enqueue_scripts() {
		// Styles.
		wp_enqueue_style( 'powerkit-slider-gallery', plugin_dir_url( __FILE__ ) . 'css/public-powerkit-slider-gallery.css', array(), powerkit_get_setting( 'version' ), 'all' );
