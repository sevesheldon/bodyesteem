...
class Powerkit_Social_Links_Admin extends Powerkit_Module_Admin {
	...
	public function contactmethods( $user_contacts ) {
		// Remove standard googleplus.
		unset( $user_contacts['googleplus'] );
	...
	public function build_options_page() {
		...
		?>
			<div class="wrap pk-social-links-wrap">
				<h2><?php esc_html_e( 'Social Links', 'powerkit' ); ?></h2>
	...
	protected function save_options_page() {
		...
		if ( isset( $_POST['save_settings'] ) ) { // Input var ok.
			...
			if ( isset( $_POST['powerkit_social_links_multiple_list'] ) ) { // Input var ok.
				update_option( 'powerkit_social_links_multiple_list', $_POST['powerkit_social_links_multiple_list'] ); // Input var ok; sanitization ok.
			} else {
			...
			if ( isset( $_POST['powerkit_social_links_order_multiple_list'] ) ) { // Input var ok.
				update_option( 'powerkit_social_links_order_multiple_list', (array) $_POST['powerkit_social_links_order_multiple_list'] ); // Input var ok; sanitization ok.
			} else {
	...
	public function admin_enqueue_scripts( $page ) {
		if ( 'settings_page_' . powerkit_get_page_slug( $this->slug ) === $page ) {
			...
			// Styles.
			wp_enqueue_style( 'powerkit-social-links', plugin_dir_url( __FILE__ ) . 'css/admin-powerkit-social-links.css', array(), powerkit_get_setting( 'version' ), 'all' );
