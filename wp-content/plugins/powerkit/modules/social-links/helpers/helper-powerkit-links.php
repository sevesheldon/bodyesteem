...
function powerkit_get_author_fields() {
	return array(
		...
		'google-plus' => esc_html__( 'Google Plus Profile URL', 'powerkit' ),
	...
	<div class="pk-author-social-links pk-social-links-wrap pk-social-links-template-<?php echo esc_html( $class ); ?>">
		<div class="pk-social-links-items">
			...
			<div class="pk-social-links-item pk-social-links-<?php echo esc_html( $data['key'] ); ?>">
				<a href="<?php echo esc_url( $data['url'] ); ?>" class="pk-social-links-link" target="_blank">
			...
			if ( $links ) {
				...
				?>
					<div class="pk-social-links-item pk-social-links-<?php echo esc_attr( $id ); ?> <?php echo esc_attr( $class ); ?>" data-id="<?php echo esc_attr( $id ); ?>">
						<a href="<?php echo esc_url( $link ); ?>" class="pk-social-links-link" target="<?php echo esc_attr( $target ); ?>" rel="<?php echo esc_attr( $rel ); ?>">
							...
							<?php if ( $params['counts'] ) { ?>
								<span class="pk-social-links-count pk-font-secondary"><?php echo esc_html( $result['count'] ); ?></span>
							<?php } ?>
							...
							<?php if ( isset( $result['error'] ) && ( current_user_can( 'editor' ) || current_user_can( 'administrator' ) ) ) { ?>
								<span class="pk-social-links-note pk-tippy">
									<?php esc_html_e( 'Error', 'powerkit' ); ?>
				...
				powerkit_alert_warning( sprintf( __( 'Please select social links in <a href="%s" target="_blank">Social Links Settings</a>.', 'powerkit' ), esc_url( powerkit_get_page_url( 'social_links' ) ) ) );
			}
			?>
		</div>
	</div>
...
function powerkit_social_links_restapi( $request ) {
	...
	foreach ( $ids as $link_id ) {
		if ( in_array( $link_id, $social_social_links, true ) ) {
			...
			$result = powerkit_social_links_get_count( $link_id, $labels, $cache, true ); // Count User.

			$class  = ( ! $result['count'] ) ? ' pk-social-links-no-count' : '';
